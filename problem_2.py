...
# Step 3: maximum a posteriori p.35
def W_maximum_a_posteriori(Lambda, Phi, t):
    '''closed-form linear regression formula'''
    return inv(Lambda * np.eye(M + 1) + Phi.T @ Phi) @ Phi.T @ t[:, None]
...
def model_train(X_train, t_train, centers, sigma_x, sigma_y, Lambda):
    '''W calculated using training set with maximum likelihood'''
    Phi_train = compute_Phi(X_train, centers, sigma_x, sigma_y)
    return W_maximum_a_posteriori(Lambda, Phi_train, t_train)
...
def s_fold_cross_validation(X, t, S, centers, sigma_x, sigma_y, Lambda):
    ...
    for i in range(S):
        ...
        W = model_train(X_train, t_train, centers, sigma_x, sigma_y, Lambda)
...
avg_mse, W, t_pred = s_fold_cross_validation(X_train, t_train, S, centers, sigma_x, sigma_y, Lambda)
print(avg_mse)
