...
# Step 3: maximum likelihood p.35
def W_maximum_likelihood(Phi, t):
    '''closed-form linear regression formula'''
    return inv(Phi.T @ Phi) @ Phi.T @ t[:, None]
...
def s_fold_cross_validation(X, t, S, centers, sigma_x, sigma_y):
    ...
    for i in range(S):
        ...
        W = model_train(X_train, t_train, centers, sigma_x, sigma_y)
...
M = 5 # number of clusters, also number of basis functions
...
avg_mse, W, t_pred = s_fold_cross_validation(X_train, t_train, S, centers, sigma_x, sigma_y)
print(avg_mse)
